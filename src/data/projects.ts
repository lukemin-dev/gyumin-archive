...
export const projects: Project[] = [
  {
    ...
    details: [
      ...
      "센서 설정과 PLC 출력 조건을 조정해 촬영 성공률을 약 80%에서 100%로 개선",
      "실제 컨베이어 운영에서 양파 8,092개의 전량 촬영을 확인하고 농산물 이미지 14,000장을 구축",
    ],
    role: "AI 모델 학습·튜닝, 데이터 라벨링·품질 검수, 센서·PLC 신호 흐름 분석과 현장 검증",
    scope: "모델 성능만 보지 않고 센서 입력 → PLC 출력 → 카메라 트리거 → AI 추론까지 전체 흐름을 담당 범위에서 점검",
    problem:
      "실제 컨베이어에서 농산물 일부가 촬영되지 않았습니다. 촬영 결과만으로는 객체 검출 모델, 카메라 트리거, PLC 출력, 센서 감지 중 어느 단계에서 누락이 발생했는지 바로 판단하기 어려웠습니다.",
    myContribution: [
      "YOLO 객체 검출 모델과 ConvNeXt 등급 분류 모델 학습·튜닝 및 실제 컨베이어 환경 평가",
      "양파와 단호박 촬영 데이터 라벨링 및 학습 데이터 품질 검수",
      "LR-Z 센서 입력, PLC 출력, 카메라 트리거, AI 추론 단계별 입력·출력 점검",
      "센서 설정과 PLC 출력 조건 조정 후 실제 운영 데이터로 촬영 결과 재검증",
    ],
    bottleneck:
    ...
    result:
      "촬영 성공률을 약 80%에서 100%로 개선했습니다. 이후 실제 컨베이어 운영에서 양파 8,092개가 모두 촬영되는 것을 확인했고, 모델 학습을 위한 농산물 이미지 14,000장을 구축했습니다.",
    validation: [
      "개선 전후 촬영 결과를 실제 컨베이어 동작과 함께 비교",
      "양파 8,092개가 통과한 실제 운영 구간에서 전량 촬영 확인",
      "센서·PLC·카메라·AI 단계의 입력과 출력을 분리해 원인 범위를 확인",
    ...
    interviewPoints: [
      ...
      "AI 모델과 설비가 결합된 시스템에서 단계별 로그를 설계하는 방법",
    ],
    techContext:
      "YOLO는 컨베이어 이미지에서 농산물을 검출하고, ConvNeXt는 촬영된 농산물의 등급을 분류하는 데 사용했습니다. 촬영 누락은 모델만으로 해결할 수 없어 LR-Z 센서와 PLC 신호까지 관찰 범위를 확장했습니다.",
    shortProblem: "농산물 일부가 촬영되지 않았지만 모델·카메라·설비 중 원인을 바로 판단하기 어려웠습니다.",
    shortAction: "센서 입력부터 PLC, 카메라 트리거, AI 추론까지 신호 흐름을 단계별로 분리해 확인했습니다.",
    shortResult: "촬영 성공률을 약 80%에서 100%로 개선하고 양파 8,092개의 전량 촬영을 확인했습니다.",
    shortQuestion: "현장 AI 시스템에서 모델과 설비 문제를 분리해 추적한 방법",
    privateCode: true,
    evidence: [
      {
        label: "현장 검증 결과",
        description: "실제 컨베이어 운영에서 양파 8,092개의 전량 촬영을 확인했습니다.",
      },
      {
        label: "데이터 구축",
        description: "양파와 단호박을 포함한 농산물 이미지 14,000장을 라벨링·검수해 학습 데이터로 정리했습니다.",
      },
      {
        label: "공개 범위",
  ...
  },
  {
    ...
    role: "Python 자동화 파이프라인 설계·구현 및 외부 API 장애 대응",
    scope: "데이터 수집·정제·우선순위 산정·AI 초안·Markdown 보고서 출력까지 연결",
    details: [
    ...
    problem:
      "클라이언트 웹사이트의 SEO 데이터를 수동으로 수집·분석·보고하는 데 약 2~3일이 걸렸습니다. 사람이 하다 보니 누락이 생기고, 담당자에 따라 분석 기준이 달라서 일관된 결과를 얻기 어려웠습니다.",
    myContribution: [
