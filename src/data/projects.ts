...
export const projects: Project[] = [
  {
    ...
    theme: "초기 100개 중 20건의 촬영·기록 누락을 LR-Z → PLC → 카메라 순으로 추적",
    details: [
      "LR-Z 센서, CUBLOC PLC P41~P44 출력, 카메라 트리거, YOLO·ConvNeXt 처리 구간을 순서대로 점검",
      "LR-Z 응답 시간을 50ms에서 10ms로 조정하고 P41~P44 출력 조건을 수정",
      "설정 변경 뒤 양파 8,092개(2,136.38kg)의 플래시 작동·데이터 기록을 모두 확인",
    ],
    role: "팀 프로젝트 · 양파·단호박 라벨 확인, YOLO·ConvNeXt 학습·튜닝, 촬영 누락 구간의 센서·PLC·카메라 흐름 점검",
    scope: "촬영 누락 대상을 기준으로 LR-Z 센서 입력 → CUBLOC PLC 출력 → 카메라 트리거 → 모델 처리 순서를 확인",
    problem:
      "초기 100개 검증에서 20개는 플래시 작동과 데이터 기록이 동시에 남지 않았습니다. 누락은 최종 이미지에서만 확인돼 센서 감지, PLC 출력, 카메라 트리거, 모델 처리 중 어디서 끊겼는지 바로 알 수 없었습니다.",
    myContribution: [
      "YOLO 객체 검출·ConvNeXt 등급 분류 모델 학습·튜닝에 참여",
      "양파·단호박 촬영 데이터의 라벨이 등급 기준과 맞는지 확인",
      "LR-Z 센서 입력, CUBLOC PLC P41~P44 출력, 카메라 트리거를 누락 대상별로 점검",
      "LR-Z의 U.C.D. 배경 튜닝·응답 시간과 P41~P44 출력 조건 변경 후 실제 운영 결과 재확인",
    ],
    bottleneck:
      "촬영 누락은 최종 이미지에서만 드러났습니다. 촬영 결과만으로는 센서 감지, PLC 출력, 카메라 트리거, 모델 처리 중 어느 단계에서 누락됐는지 알 수 없었습니다.",
    solution:
      "플래시가 작동하지 않은 대상을 기준으로 LR-Z 입력, PLC P41~P44 출력, 카메라 트리거를 거슬러 올라갔습니다. LR-Z를 U.C.D. 배경 튜닝 모드로 바꾸고 응답 시간을 50ms에서 10ms로 조정했으며, P41~P44 출력 조건을 수정했습니다.",
    result:
      "초기 검증에서는 100개 중 80개만 플래시 작동과 데이터 기록이 모두 남았습니다. 설정 변경 후 실제 컨베이어 운영에서는 양파 8,092개(2,136.38kg) 모두에서 같은 두 기록을 확인했습니다. 모델 학습에는 기존 농산물 이미지 약 14,000장을 사용했습니다.",
    validation: [
      "성공 기준을 플래시 작동과 데이터 기록이 동시에 남는지로 정하고, 초기 100개 중 80개를 집계",
      "설정 변경 후 양파 8,092개(2,136.38kg)가 통과한 실제 운영 구간에서 같은 기준을 모두 충족하는지 확인",
      "LR-Z 입력, PLC 출력, 카메라 트리거, 모델 처리 구간을 분리해 확인",
      "보안상 원본 설비 로그와 코드는 공개하지 않고 검증 절차와 결과만 공개",
    ],
    retrospective:
      "로그와 실제 컨베이어 동작을 함께 보며 센서 입력, PLC 출력, 촬영 결과를 순서대로 비교했습니다. 다시 한다면 농산물마다 같은 식별자를 부여해 센서 감지부터 추론 결과까지 한 번에 비교하겠습니다.",
    interviewPoints: [
      "촬영 누락이 보일 때 모델뿐 아니라 센서·PLC·카메라 구간까지 확인한 과정",
      "촬영 성공률과 8,092개 전량 촬영을 확인한 현장 검증 방식",
      "단계별 로그와 컨베이어 동작을 대조해 확인 범위를 좁힌 방법",
    ],
    techContext:
      "YOLO는 컨베이어 이미지에서 농산물을 검출하고 ConvNeXt는 등급을 분류했습니다. 촬영 결과가 없을 때에는 모델뿐 아니라 LR-Z 입력과 PLC·카메라 출력도 함께 확인했습니다.",
    shortProblem: "초기 100개 중 20개에서 플래시 작동과 데이터 기록이 동시에 남지 않았습니다.",
    shortAction: "누락 대상별로 LR-Z 입력 → PLC P41~P44 출력 → 카메라 트리거 순서로 확인했습니다.",
    shortResult: "설정 변경 뒤 실제 운영에서 양파 8,092개(2,136.38kg)의 플래시 작동·데이터 기록을 모두 확인했습니다.",
    shortQuestion: "현장 AI 시스템에서 모델과 설비 문제를 분리해 추적한 방법",
    privateCode: true,
    evidence: [
      {
        label: "현장 검증 결과",
        description: "설정 변경 후 실제 컨베이어 운영에서 양파 8,092개(2,136.38kg)의 플래시 작동과 데이터 기록을 모두 확인했습니다.",
      },
      {
        label: "데이터 활용",
        description: "약 14,000장 규모로 확보되어 있던 농산물 이미지 데이터셋을 모델 학습·튜닝에 활용하고 라벨과 품질을 확인했습니다.",
      },
      {
        label: "공개 범위",
        description: "연구기관 설비와 데이터 보안을 위해 원본 코드·로그 대신 신호 흐름, 담당 범위와 검증 결과를 공개합니다.",
      },
    ],
    featured: true,
  },
