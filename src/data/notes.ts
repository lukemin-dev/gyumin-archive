...
export const notes: Note[] = [
  {
    ...
    date: "2026-07-28",
    summary:
      "촬영되지 않은 농산물을 따라가며 LR-Z 센서부터 PLC 출력, 카메라 트리거까지 순서대로 확인했습니다.",
...
이 일을 겪은 뒤에는 화면에 결과가 없을 때 모델부터 의심하지 않습니다. 센서가 감지했는지, PLC 출력이 나갔는지, 카메라 트리거가 발생했는지를 차례로 확인하는 순서가 생겼습니다.`,
  },
  {
...
Crosslink 인턴십에서 SEO 자동화 파이프라인을 만들 때 입력 데이터의 형식이 달라지고 Gemini API 할당량 초과와 응답 지연이 반복됐습니다. 입력값 검증과 예외 데이터 분리를 넣고, 실패한 단계만 다시 실행할 수 있도록 작업을 나누었습니다. 중간 결과는 체크포인트로 저장했습니다.
