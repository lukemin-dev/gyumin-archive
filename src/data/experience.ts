...
export const experiences: Experience[] = [
  {
    ...
    period: "재직 중",
    context:
      "농산물이 컨베이어를 지나가면 센서가 감지하고 PLC가 카메라를 작동시키며, 촬영된 이미지를 AI 모델이 판정하는 시스템을 다루고 있습니다.",
    responsibility:
      "YOLO 객체 검출 모델과 ConvNeXt 등급 분류 모델을 학습·튜닝하고 실제 컨베이어에서 결과를 확인하고 있습니다. 양파와 단호박 촬영 데이터를 등급 기준에 맞춰 라벨링하고 학습 데이터의 품질도 살펴봅니다.",
    problemEncountered:
      "초기에는 컨베이어를 통과한 농산물이 일부 촬영되지 않았지만, 모델 문제인지 카메라 문제인지 설비 신호 문제인지 한 번에 판단하기 어려웠습니다.",
    actionTaken:
      "플래시가 터지지 않은 구간을 기준으로 KEYENCE LR-Z 센서 입력, PLC 출력, 카메라 트리거를 차례로 확인했습니다. 로그의 신호와 실제 컨베이어 동작도 함께 비교했습니다.",
    result:
      "처음 확인한 100개 중 플래시와 데이터 기록이 모두 남은 것은 80개였습니다. 센서 설정을 바꾼 뒤 실제 운영 구간에서는 양파 8,092개 모두 같은 기준을 충족했습니다.",
    whatILearned:
      "처음에는 촬영 누락을 모델 문제로 생각하기 쉬웠지만, 실제 원인은 모델 앞단의 센서와 PLC에도 있을 수 있었습니다. 화면의 결과만 보지 않고 장비 신호를 거슬러 올라가는 방법을 익히고 있습니다.",
